...
import { laocaiHotlineGroups } from '../../data/laocaiMockData';

const ALL_KEY = 'tat-ca';
const TABS = [
    { key: ALL_KEY, label: 'Tất cả', items: laocaiHotlineGroups.flatMap((g) => g.items.map((i) => ({ ...i, group: g.key }))) },
    ...laocaiHotlineGroups.map((g) => ({ ...g, items: g.items.map((i) => ({ ...i, group: g.key })) }))
];
...
const LaoCaiHotlinePage = () => {
    ...
    useEffect(() => {
        document.title = 'Hotline - Đường dây nóng - Cổng Pháp luật Lào Cai';
        window.scrollTo(0, 0);
    }, []);
        ...
        <div className="font-sans min-h-screen flex flex-col bg-[#f4f7fb]">
            ...
            <div className="bg-white border-b border-gray-200">
                <div className="container mx-auto px-4 max-w-[1286px] py-3 text-xs sm:text-sm text-gray-500 flex items-center gap-2">
                    <Link to="/lao-cai" className="hover:text-[#2c1b92] transition-colors">Trang chủ Lào Cai</Link>
            ...
            <div className="relative text-white py-8 sm:py-10 overflow-hidden bg-gradient-to-r from-[#4f56ca] via-[#2c1b92] to-[#4f56ca] border-b border-indigo-400/30">
                ...
                <div className="container mx-auto px-4 max-w-[1286px] relative z-10">
                    ...
                    <p className="text-xs sm:text-sm text-amber-50/95 mt-1 max-w-3xl leading-relaxed drop-shadow-sm font-normal">
                        Danh bạ số điện thoại đường dây nóng của Sở Tư pháp, các Trung tâm Trợ giúp pháp lý và tổ chức hành nghề luật sư trên địa bàn tỉnh Lào Cai
                    </p>
                </div>
            </div>
...
export default LaoCaiHotlinePage;
