...
import { hanoiLegalDocs, hanoiDraftDocs, hanoiNewsArticles } from '../../data/hanoiMockData';

const HanoiDocsAndNews = () => {
    const [activeTab, setActiveTab] = useState('docs'); // 'docs' | 'drafts' | 'news'
    const [filterField, setFilterField] = useState('all');
    const [searchFilter, setSearchFilter] = useState('');

    const filteredDocs = hanoiLegalDocs.filter(d => {
    ...
    return (
        <section id="van-ban-ha-noi" className="py-14 bg-white">
            <div className="max-w-[1400px] mx-auto px-4">
                ...
                <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between pb-6 border-b border-gray-200 gap-4 mb-8">
                    <div>
                        ...
                        <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 tracking-tight">
                            Hệ Thống Văn Bản & Hoạt Động Pháp Luật Thủ Đô
                        </h2>
                    </div>
                    ...
                    <div className="flex bg-gray-100 p-1 rounded-xl gap-1 text-xs sm:text-sm font-bold w-full sm:w-auto">
                        ...
                        >
                            ...
                            <span className={`text-[10px] px-1.5 py-0.2 rounded-full ${activeTab === 'news' ? 'bg-white/20 text-white' : 'bg-gray-200 text-gray-700'}`}>
                                {hanoiNewsArticles.length}
                            </span>
                        </button>
                    </div>
                </div>

                {/* Tab 1: Legal Documents of Hanoi */}
                {activeTab === 'docs' && (
                        ...
                        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-gray-50 p-3 rounded-xl border border-gray-200">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                >
                                    ...
                                    <option value="Đất đai">Đất đai - Đô thị</option>
                                    <option value="Đầu tư">Đầu tư - Doanh nghiệp</option>
                                    <option value="Môi trường">Môi trường - Giao thông</option>
                ...
                {activeTab === 'drafts' && (
                    <div className="space-y-4">
                        <div className="bg-amber-50 border border-amber-200 text-amber-900 text-xs sm:text-sm p-4 rounded-xl flex items-center gap-3">
                            ...
                            <p>
                                Mọi công dân, tổ chức và chuyên gia Thủ đô đều có quyền tham gia đóng góp ý kiến xây dựng các dự thảo Nghị quyết HĐND và Quyết định UBND Thành phố Hà Nội nhằm hoàn thiện chính sách phát triển Thủ đô.
                            </p>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                            {hanoiDraftDocs.map((draft) => (
                                <div 
                                    key={draft.id}
                ...
                {/* Tab 3: News & Legal Education in Hanoi */}
                {activeTab === 'news' && (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {hanoiNewsArticles.map((art) => (
                            <div 
                                key={art.id}
...
export default HanoiDocsAndNews;
