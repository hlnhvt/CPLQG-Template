...
import { laocaiHotlineGroups, laocaiConnectedPortals } from '../../data/laocaiMockData';

const TAB_ICONS = { 'so-tu-phap': Landmark, 'tro-giup-phap-ly': Scale, 'luat-su': Briefcase };
const HOTLINE_TABS = laocaiHotlineGroups.map((g) => ({ ...g, icon: TAB_ICONS[g.key] }));
...
const LaoCaiHotline = () => {
    ...
    return (
        <section id="hotline" className="py-6 laocai-animate-fade-up" style={{ animationDelay: '0.45s' }}>
            <div className="container mx-auto px-4 max-w-[1504px] grid grid-cols-1 lg:grid-cols-10 gap-6">
                ...
                <div className="lg:col-span-7 bg-white rounded-2xl p-5 sm:p-6 border border-gray-200/80 shadow-sm">
                    {/* Header Khối Hotline phong cách đồng bộ Banner Cổng Lào Cai (#4f56ca -> #2c1b92 -> #4f56ca) */}
                    <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-[#4f56ca] via-[#2c1b92] to-[#4f56ca] p-3 sm:p-3.5 mb-4 text-white shadow-md border-b-2 border-amber-400">
                ...
                <aside className="lg:col-span-3 bg-white rounded-2xl p-5 border border-gray-200/80 shadow-sm flex flex-col">
                    ...
                    <div className="flex-1 min-h-0 relative">
                        <ul className="lg:absolute lg:inset-0 overflow-y-auto pr-1 space-y-2">
                            {laocaiConnectedPortals.map((portal) => (
                                <li key={portal.id}>
                                    <a
