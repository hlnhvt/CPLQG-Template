...
import { laocaiLegalDocs } from '../../data/laocaiMockData';

const LaoCaiNewlyIssuedDocs = () => {
    const docs = laocaiLegalDocs.map(doc => ({
        date: doc.ngayBanHanh,
        title: `${doc.soHieu} (${doc.coQuan}) - ${doc.trichYeu}`
    }));

    return (
        <section id="van-ban-moi" className="py-1 laocai-animate-fade-up" style={{ animationDelay: '0.25s' }}>
            <div className="container mx-auto px-4 max-w-[1504px]">
                {/* Header Khối Văn Bản Mới Ban Hành phong cách đồng bộ Banner Cổng Lào Cai (#4f56ca -> #2c1b92 -> #4f56ca) */}
...
export default LaoCaiNewlyIssuedDocs;
