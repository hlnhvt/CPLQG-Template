...
import { laocaiV2LegalDocs, laocaiV2DraftDocs, laocaiV2NewsArticles } from '../../data/laocaiV2MockData';

const LaoCaiV2DocsAndNews = () => {
    const [activeTab, setActiveTab] = useState('docs'); // 'docs' | 'drafts' | 'news'
    const [filterField, setFilterField] = useState('all');
    const [searchFilter, setSearchFilter] = useState('');

    const filteredDocs = laocaiV2LegalDocs.filter(d => {
    ...
    return (
        <section id="van-ban-lao-cai" className="py-14 bg-white">
            <div className="max-w-[1400px] mx-auto px-4">
                    ...
                    <div className="flex bg-gray-100 p-1 rounded-xl gap-1 text-xs sm:text-sm font-bold w-full sm:w-auto">
                        ...
                        >
                            ...
                            <span className={`text-[10px] px-1.5 py-0.2 rounded-full ${activeTab === 'news' ? 'bg-white/20 text-white' : 'bg-gray-200 text-gray-700'}`}>
                                {laocaiV2NewsArticles.length}
                            </span>
                        </button>
                    </div>
                </div>

                {/* Tab 1: Legal Documents of LaoCaiV2 */}
                {activeTab === 'docs' && (
                ...
                {/* Tab 3: News & Legal Education in LaoCaiV2 */}
                {activeTab === 'news' && (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {laocaiV2NewsArticles.map((art) => (
                            <div 
                                key={art.id}
...
export default LaoCaiV2DocsAndNews;
