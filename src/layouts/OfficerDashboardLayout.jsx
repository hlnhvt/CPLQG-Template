...
const OfficerDashboardLayout = () => {
    ...
    useEffect(() => {
        localStorage.setItem('officerSidebarCollapsed', isCollapsed);
    }, [isCollapsed]);
    ...
    const isHomePage = locationToUse.pathname === '/can-bo/trang-chu';
                ...
                <div className="bg-white border-b border-gray-200 pt-4 pb-4 shadow-sm z-10 relative">
                    <div className="container mx-auto px-4 max-w-[1500px]">
                        <div className="flex items-center text-[13px] text-gray-500 whitespace-nowrap overflow-x-auto">
                            ...
                            <span className="text-gray-800 font-medium">Khu vực cán bộ</span>
                        </div>
                    </div>
                </div>
            )}
                                ...
                                <div className="flex flex-col md:flex-row justify-between gap-4">
                                    <div>
                                        ...
                                        <p className="text-[13px] text-gray-500 flex items-center gap-1.5">
                                            <MapPin size={14} className="text-gray-400" /> Cầu Giấy, Hà Nội &middot; <Link to="/can-bo/ho-so" className="text-blue-600 font-medium hover:underline">Chi tiết liên hệ</Link>
                                        </p>
                                    </div>
                                    ...
                                    <div className="flex md:flex-col justify-between md:justify-start items-center md:items-end gap-3 shrink-0">
                                        ...
                                        <div className="flex gap-2">
                                            <Link to="/can-bo/ho-so" className="w-8 h-8 flex items-center justify-center rounded-full text-gray-500 hover:text-blue-600 hover:bg-blue-50 transition-colors" title="Chỉnh sửa hồ sơ">
                                                <Edit3 size={16} />
                                            </Link>
                                            <button className="w-8 h-8 flex items-center justify-center rounded-full text-gray-500 hover:text-blue-600 hover:bg-blue-50 transition-colors" title="Chia sẻ trang cán bộ">
                                                <Share2 size={16} />
                                            </button>
                                        </div>
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden sticky top-8">
                        ...
                        <div className={`bg-[#0f4c81] p-4 flex items-center transition-all duration-300 ${isCollapsed ? 'flex-col justify-center py-5' : 'justify-between'}`}>
                            <div className={`transition-all duration-300 overflow-hidden whitespace-nowrap ${isCollapsed ? 'w-0 opacity-0 h-0 hidden' : 'w-auto opacity-100'}`}>
                                <h2 className="text-white font-bold text-lg mb-0.5 leading-tight">Khu vực cán bộ</h2>
                                <p className="text-white/80 text-[12px]">Quản lý công việc chung</p>
                            </div>
